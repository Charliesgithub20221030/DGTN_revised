...
parser.add_argument('--id', type=int, default=100, help='id')
...
def main():
    ...
    cur_dir = os.getcwd()
    train_dataset = MultiSessionsGraph(pj(cur_dir , 'datasets' , opt.dataset), phrase='train', knn_phrase='neigh_data_'+str(opt.id))
    train_loader = DataLoader(train_dataset, batch_size=opt.batch_size,num_workers=4, shuffle=True , pin_memory= True)
    test_dataset = MultiSessionsGraph(pj(cur_dir , 'datasets' ,opt.dataset), phrase='test', knn_phrase='neigh_data_'+str(opt.id))
    test_loader = DataLoader(test_dataset, batch_size=opt.batch_size,num_workers=4, shuffle=False, pin_memory= True)
    ...
    log_dir = pj(cur_dir  ,'log' , str(opt.dataset) , time.strftime(
    ...
    if not opt.predict:
        ...
        log_dir = pj('log','cikm16','2019-08-19 14:27:33')
        model.load_state_dict(torch.load(pj(log_dir,'best_mrr_params.pkl')))
